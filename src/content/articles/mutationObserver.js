  ...
  render() {
    return (
      <article>
        ...
        <section>
          <h2>Встурпление (Проблема)</h2>
          <p>
            Предположим что есть минифицированная внешняя библиотека, которую асинхронно загружаем на одной конкретной странице,
            поскольку она имеет узкую направленность и больше нигде на сайте не нужна. Библиотечка принимает на вход
            контейнер внутрь которого будет рендерить свое тело, например аккордион с формой, условиями
            договора, прочей информацией или интерактивными элементами.
          </p>
          <p>
            Ну а мы хотим сделать что-то сразу после загрузки, например отправить пользователю уведомление или сделать
            инпут активным. Однако когда закончится рендер? Мы можем отследить конец
